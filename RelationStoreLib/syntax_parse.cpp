...
namespace RelationStoreLib
{
    ...
    */
    ...
    std::string ArgPackToScriptArg(const DoConnectionOpArgPack &arg, const std::string &opName, const std::string &relationName)
    {
        auto vecFloatToVecString = [](const std::vector<float> &fs)
        {
            return fs | std::views::transform([](float f)
                                              {
                                                  return std::format("{}", f); // 类似于 C# 的 f.ToString()
                                              }) |
        ...
        return std::format("{}({}),[{}]", opName, shizuku::util::string::Join(sa, ","), vecFloatToVecString(arg.Weights));
    }
    ...
    public:
        ...
        {
            return ArgPackToScriptArg<GraphType::Uni>(_ArgPack, OpName, RelationName);
        }
    };
    ...
    public:
        ...
        std::string ToString() override
        {
            return ArgPackToScriptArg<GraphType::Bid>(_ArgPack, OpName, RelationName);
        }
    };
